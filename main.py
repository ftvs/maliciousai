...
def eval_sadtalker(dataset_path,batch_size,device,model_path):
    ...
    test_dataset = CelebDF2(dataset_path, transform=transform, max_frames=300, n_frames=150,file_list = 'List_of_testing_videos.txt') # 10s @ 30fps = 300 frames, sample 15 frames per 1s (60,100,150)
    print(f"Test Dataset Size: {len(test_dataset)}")

    test_loader = DataLoader(test_dataset, batch_size, pin_memory=True, num_workers=4, persistent_workers=True, shuffle=False) #, num_workers=4, persistent_workers=True

    first_data, first_labels = next(iter(test_loader))
    print(f"Input Shape: {first_data.shape}")
    print(f"label Shape: {first_labels.shape}")
    ...
    outputs1, all_labels, test_loss, test_accuracy, test_cm = infer(model, test_loader, loss_fn, device)

    return test_loss, test_accuracy, test_cm

def eval_ensemble(dataset_path1,dataset_path2, batch_size, device, model_path1,model_path2):
    # Set the random seed for reproducibility
    seed = 50  # You can choose any integer for your seed value
    ...
    test_dataset1 = CelebDF2(dataset_path1, transform=transform, max_frames=300, n_frames=150,file_list = 'List_of_testing_videos.txt') # 10s @ 30fps = 300 frames, sample 15 frames per 1s (60,100,150)
    test_dataset2 = CelebDF2(dataset_path2, transform=transform, max_frames=300, n_frames=150,file_list = 'List_of_testing_videos.txt')
    print(f"Test Dataset 1 Size: {len(test_dataset1)}")
    print(f"Test Dataset 2 Size: {len(test_dataset2)}")

    test_loader1 = DataLoader(test_dataset1, batch_size, pin_memory=True, num_workers=4, persistent_workers=True, shuffle=False) #, num_workers=4, persistent_workers=True
    test_loader2 = DataLoader(test_dataset2, batch_size, pin_memory=True, num_workers=4, persistent_workers=True, shuffle=False)

    first_data, first_labels = next(iter(test_loader1))
    print(f"Input Shape: {first_data.shape}")
    print(f"label Shape: {first_labels.shape}")
    ...
    # load saved model
    checkpoint1 = torch.load(model_path1, weights_only=False)

    model1 = checkpoint1['model']
    freeze(model1)

    checkpoint2 = torch.load(model_path2, weights_only=False)
    ...
    test_loss, test_accuracy, test_cm = ensemble(model1, model2, test_loader1, test_loader2, loss_fn, device)
